...
class ActiveField extends \yii\widgets\ActiveField
{
    ...
    public function checkbox($options = [], $enclosedByLabel = true)
    {
        ...
        if ($enclosedByLabel) {
            ...
            if (isset($options['label'])) {
                $showVar = '';
                if($annotated){
                    $showVar = "<code>{$this->attribute}</code>";
                }
                $this->parts['{labelTitle}'] = $options['label'] . ' ' . $showVar;
            }
    ...
    public function checkboxList($items, $options = [])
    {
        if ($this->inline) {
            ...
            $options['item'] = function ($index, $label, $name, $checked, $value) {
                return '<div class="checkbox">' . Html::checkbox($name, $checked, ['label' => "<span>$label</span>", 'value' => $value]) . '</div>';
            };
        }
    ...
    public function radioList($items, $options = [])
    {
		...
		$options['item'] = function ($index, $label, $name, $checked, $value, $other, $showVar, $encode, $options) {
                    
                    $itemOptions = isset($options['itemOptions']) ? $options['itemOptions'] : [];
                    if (isset($options['disabled'])) {
                        $itemOptions['disabled'] = $options['disabled'];
                    }
                    
		    $other_input = ((isset($other))?Html::activeTextInput($this->model, $other['attribute'], ['class'=>'form-control']):'') . ((isset($other))?" {$other['suffix']}":'');
                    if(isset($other['type']) && $other['type']=='select'){
                        ...
                        if ($dataEzf) {
                            ...
                            if ($nameConcat) {
                                ...
                                $other_input = Html::activeDropDownList($this->model, $other['attribute'], ArrayHelper::map($data, 'id', 'name'), ['class'=>'form-control', 'prompt'=>'Please select ...']);
                            }
                        } 
		    ...
		    return $beginTag.Html::radio($name, $checked, array_merge($itemOptions,[
				'labelOptions' => ['class' => 'radio-inline'],
				'value' => $value,
				'label' => '<span>'.($encode ? \yii\bootstrap\Html::encode($label) : $label).' '.$other_input.' '.$showVar.'</span> ',
		    ])).$endTag;
		};
	    }
        }  elseif (!isset($options['item'])) {
		...
		if(isset($other['attribute'])){
                    $other_id = Html::getInputId($this->model, $other['attribute']);
                    $js = "
                        if($('input[name=\"{$name}\"][type=\"radio\"]').filter(':checked').val() == '$value') {
                            $('#$other_id').attr('readonly', false);
                          } else {
                            $('#$other_id').attr('readonly', true);
                          }
                        
                        $('input[name=\"{$name}\"][type=\"radio\"]').click(function(){
                              let v = $(this).filter(':checked').val();
                              if(v == '$value') {
                                $('#$other_id').attr('readonly', false);
                                $('#$other_id').focus();    
                              } else {
                ...
                return '<div class="radio">' . Html::radio($name, $checked, array_merge($itemOptions,['label' => "<span>$label $other_input $showVar</span>", 'value' => $value])) . '</div>';
            };
        }
        //parent::radioList($items, $options);
	$this->adjustLabelFor($options);
        $this->parts['{input}'] = EzformWidget::activeRadioList($this->model, $this->attribute, $items, $options);
