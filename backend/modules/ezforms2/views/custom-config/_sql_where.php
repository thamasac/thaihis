...
<div class="row" >
  ...
  <div class="col-md-1 sdbox-col"><label>Function</label></div>
  <div class="col-md-2 sdbox-col"><label>Field</label></div>
  <div class="col-md-1 sdbox-col"><label>Condition</label></div>
...
<div class="row item " data-id="<?=$index_id?>">
  ...
  <div class="col-md-2 sdbox-col tbfields-<?=$index_id?>">
      
    </div>
  <div class="col-md-1 sdbox-col">
      <?= Html::dropDownList("{$name}[{$index_id}][cond]", isset($options['cond'])?$options['cond']:'=', ModuleFunc::itemAlias('sqlCondition'), ['class'=>'form-control options-cond', 'id'=>"cond-$index_id"])?>
    </div>
  ...
  <div class="col-md-2 sdbox-col">
      <?= Html::textInput("{$name}[{$index_id}][value2]", isset($options['value2'])?$options['value2']:'', ['class'=>'form-control options-value2', 'id'=>"value2-$index_id", 'readonly'=>(isset($options['cond']) && $options['cond']=='BETWEEN')?FALSE:TRUE, 'maxlength'=>255])?>
    </div>
  <div class="col-md-1 sdbox-col">
  ...
  <div class="col-md-1 sdbox-col">
      <?= Html::dropDownList("{$name}[{$index_id}][with]", isset($options['with'])?$options['with']:'AND', ModuleFunc::itemAlias('sqlAndOr'), ['class'=>'form-control options-with', 'id'=>"with-$index_id"])?>
    </div>
    <div class="col-md-1 sdbox-col">
...
function tableFields(ezf_id){
    let value = '$field';
    $.post('". \yii\helpers\Url::to(['/ezforms2/target/table-fields'])."',{ ezf_id: ezf_id, multiple:0, name: '{$name}[{$index_id}][field]', value: value ,id:'field-$index_id'}
