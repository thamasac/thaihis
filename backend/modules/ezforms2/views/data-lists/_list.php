...
<div id="selete-ezform-fav">
    <div class="modal-header">
        ...
        <h4 class="modal-title" id="itemModalLabel"><?= Yii::t('ezform', 'Favorite Forms')?></h4>
    </div>
    ...
    $items = [
        [ 
            ...
            'url' => yii\helpers\Url::to(['/ezforms2/data-lists/list', 'tab' => '1']),
            'encode'=>false,
            'active' => $tab == '1',
        ...
        [
            ...
            'url' => yii\helpers\Url::to(['/ezforms2/data-lists/list', 'tab' => '2']),
            'encode'=>false,
            'active' => $tab == '2',
            'template' => '{update}'
        ],
        [
            ...
            'url' => yii\helpers\Url::to(['/ezforms2/data-lists/list', 'tab' => '3']),
            'active' => $tab == '3',
            'encode'=>false,
            'template' => '{view}'
        ],
        [
            ...
            'url' => yii\helpers\Url::to(['/ezforms2/data-lists/list', 'tab' => '4']),
            'active' => $tab == '4',
            'encode'=>false,
    ...
    echo GridView::widget([
        ...
        'columns' => [
            [
                'attribute' => 'ezf_id',
                'label'=> Yii::t('ezform', 'Favorite Forms'),
                'value'=> function ($data) use ($modelFav){
                    if(isset($modelFav) && !empty($modelFav)){
                        foreach ($modelFav as $value) {
                            if($value['ezf_id'] == $data['ezf_id']){
                                return yii\helpers\Html::a('<i class="glyphicon glyphicon-star"></i>', \yii\helpers\Url::to(['/ezforms2/data-lists/favorite', 'ezf_id'=>$data['ezf_id']]),['style'=>'font-size: 30px;color:#17a2b8;']);
                            }
                        }
                    }
                    
                    return yii\helpers\Html::a('<i class="glyphicon glyphicon-star-empty"></i>', \yii\helpers\Url::to(['/ezforms2/data-lists/favorite', 'ezf_id'=>$data['ezf_id']]),['style'=>'font-size: 30px;color:#17a2b8;']);
                },
                'format'=>'raw',        
                'filter' => '',
...
$('#ezform-fav-grid tbody tr td a').on('click', function() {
    ...
    var star = $(this).find('i').hasClass('glyphicon-star');
    ...
    if(star){
        $(this).find('i').removeClass('glyphicon-star');
        $(this).find('i').addClass('glyphicon-star-empty');
    } else {
        $(this).find('i').addClass('glyphicon-star');
        $(this).find('i').removeClass('glyphicon-star-empty');
    }
