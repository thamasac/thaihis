...
<p class="lead" style="font-size: 16px;margin-bottom: 10px;"> <?=Yii::t('ezmodule', 'Total')?> <span id="count-<?=$object_id?>-<?=$query_tool?>"></span> <?=Yii::t('ezmodule', 'items')?></p>
<div id="<?=$comment_box?>" class="well" style="background-color: #fff;"></div>
...
function getCommentList() {
    $('#more_comment-$object_id-$query_tool').attr('data-start', '$limit');
            ...
            $.ajax({
                method: 'GET',
                url: '" . Url::to(['/ezforms2/ezform-community/comment-list', 
                    'dataid' => $dataid,
                    'object_id' => $object_id,
                    'query_tool' => $query_tool,
                    'parent_id' => $parent_id,
...
function getMoreCommentList(start) {
    $.ajax({
        method: 'GET',
        url: '" . yii\helpers\Url::to(['/ezforms2/ezform-community/comment-list', 
            'dataid' => $dataid,
            'object_id' => $object_id,
            'query_tool' => $query_tool,
            'parent_id' => $parent_id,
