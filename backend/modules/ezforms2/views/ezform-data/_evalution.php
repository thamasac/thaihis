...
$ezformAll = backend\modules\ezforms2\classes\EzfQuery::getEzformListByGroup($category);
...
//        'template' => '{actions}',
//        'buttons' => [
//            'actions' => function ($url, $data, $key) use($reloadDiv, $modal) {
//                $html = '';
...
//                    $html .= Html::a('<span class="glyphicon glyphicon-eye-open"></span>', Url::to(['/ezforms2/ezform-data/ezform-view',
//                                        'ezf_id' => $ezf_id,
//                                        'dataid' => $data['data_id'],
//                                        'modal' => $modal,
//                                        'reloadDiv' => $reloadDiv,
...
//                    $html .= Html::a('<span class="glyphicon glyphicon-pencil"></span>', Url::to(['/ezforms2/ezform-data/ezform',
//                                        'ezf_id' => $ezf_id,
//                                        'dataid' => $data['data_id'],
//                                        'modal' => $modal,
//                                        'reloadDiv' => $reloadDiv,
...
//                    $html .= Html::a('<span class="glyphicon glyphicon-trash"></span>', Url::to(['/ezforms2/ezform-data/delete',
//                                        'ezf_id' => $ezf_id,
//                                        'dataid' => $data['data_id'],
//                                        'reloadDiv' => $reloadDiv,
//                                    ]), [
...
$columns[] = [
    ...
    'contentOptions' => ['style' => 'width:140px;min-width:100px;'],
    'filter' => \kartik\daterange\DateRangePicker::widget([
                        'model'=>$searchModel,
                        'attribute'=>'create_date',
                        'convertFormat'=>true,
                        //'useWithAddon'=>true,
                        'options'=>['id'=>'evaldate_'.$reloadDiv.'_'. $category, 'class'=>'form-control'],
                        'pluginOptions'=>[
                            'locale'=>[
                                'format'=>'d-m-Y',
                                'separator'=>' to ',
    ...
    'value' => function ($data) use($reloadDiv, $modal, $disabled) {
        ...
        if (is_array($detail)) {
            ...
            if($zdata){
                ...
                $html = '<div class="pull-right text-right">'.$actions.'</div>  <h4 class="page-header">'.$data['ezf_name'].' <small> By <i class="glyphicon glyphicon-user"></i> '.$data['userby'].' '.(!empty($data['update_date']) ? '<i class="glyphicon glyphicon-calendar"></i> '.\appxq\sdii\utils\SDdate::mysql2phpDateTime($data['update_date'],'-') : '').'</small></h4>';
                $comma = '';
...
$this->registerJs("
$('#$reloadDiv-evalution-grid tbody tr').on('dblclick', function() {
    let id = $(this).attr('data-key');
    $(this).find('.btn-update').click();
});

$('#$reloadDiv-evalution-grid tbody tr td a').on('click', function() {
...
$('#$reloadDiv-evalution-grid').on('beforeFilter', function(e) {
...
$('#$reloadDiv-evalution-grid .pagination a').on('click', function() {
    getUiAjax($(this).attr('href'), '$reloadDiv');
    return false;
});

$('#$reloadDiv-evalution-grid thead tr th a').on('click', function() {
    getUiAjax($(this).attr('href'), '$reloadDiv');
    return false;
});
