...
?>
<?= \yii\helpers\Html::hiddenInput('options[render]', '/ezmodule-tab/ezcalendar/widget'); ?>
...
<div class="row" style="margin-top: 15px; margin-bottom: 15px;">
    <div class="col-md-3">
      <label class="control-label">Default View </label>
        <?= Html::dropDownList('options[params][defaultView]', isset($options['params']['defaultView'])?$options['params']['defaultView']:'month', ['month'=>'Month', 'agendaWeek'=>'Week', 'agendaDay'=>'Day', 'listWeek'=>'List', 'basicWeek'=>'Basic'], ['class'=>'form-control'])?>
    </div>
    <div class="col-md-9">
      <label class="control-label">View Menu</label>
        <?= Html::checkboxList('options[params][view_menu]', isset($options['params']['view_menu'])?$options['params']['view_menu']:['month', 'agendaWeek', 'agendaDay'], ['month'=>'Month', 'agendaWeek'=>'Week', 'agendaDay'=>'Day', 'listWeek'=>'List', 'basicWeek'=>'Basic'])?>
    </div>  
</div>
<hr>
<div class="row" >
...
<div id="dad-item-box-tab" class="forms-items-tab">
    <?php
    if (isset($options['params']['forms']) && is_array($options['params']['forms']) && !empty($options['params']['forms'])) {
        ...
        foreach ($options['params']['forms'] as $key_form => $value_form) {
          ...
          <div id="<?= $key_form ?>" class="row form-fields dads-children" data-id="<?= $key_form ?>" style="margin-bottom: 15px;">
            ...
            <div class="col-md-3 " style="padding-top: 10px;"><input type="text" class="form-control label-input" name="options[params][forms][<?= $key_form ?>][label]" value="<?=$value_form['label']?>" placeholder="label"></div>

            <div class="col-md-1 sdbox-col" style="margin-top: 15px; padding-bottom: 10px;">
              <div class="input-group">
                  <?= Html::textInput("options[params][forms][$key_form][color]", $value_form['color'], ['class' => 'form-control input-color', 'id' => 'specific-color-' . $key_form]) ?>
              </div>
            </div>
            ...
            <div class="col-md-2 sdbox-col" style="margin-top: 15px;"><?= Html::checkbox("options[params][forms][$key_form][show]", isset($value_form['show'])?$value_form['show']:0, ['label' => 'Show']) ?></div>
            <div class="col-md-2 sdbox-col" style="margin-top: 15px;"><?= Html::checkbox("options[params][forms][$key_form][editable]", isset($value_form['editable'])?$value_form['editable']:0, ['label' => 'Edit Data']) ?></div>
          </div>
...
//    });
    ...
    function getWidget_tab() {
        $.ajax({
            method: 'POST',
            url: '" . Url::to(['/ezmodules/ezmodule-tab/get-widget', 'view' => 'ezcalendar/_form_item']) . "',
            dataType: 'HTML',
            success: function(result, textStatus) {
                $('#dad-item-box-tab').append(result);
                //$('#dad-item-box-tab').removeClass('dads-children');
            }
        });
    }
");
