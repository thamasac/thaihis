...
<div class="form-group row">
  ...
  <div class="col-md-3 sdbox-col">
     <?= Html::hiddenInput('options[filter]', 0)?>
      <?= backend\modules\ezforms2\classes\EzformWidget::checkbox('options[filter]', isset($options['filter'])?$options['filter']:0, ['label'=> Yii::t('ezmodule', 'Show Filter')])?>
  ...
  <div class="col-md-3 sdbox-col">
      ...
      <?= backend\modules\ezforms2\classes\EzformWidget::checkbox('options[addbtn]', isset($options['addbtn'])?$options['addbtn']:0, ['label'=> Yii::t('ezmodule', 'Show Add Button')])?>
...
<div class="form-group">
  <div class="row">
    ...
    <div class="col-md-3 sdbox-col"><label><?= Yii::t('ezform', 'Header Label')?></label></div>
    <div class="col-md-2 sdbox-col"><label><?= Yii::t('ezform', 'Align')?></label></div>
    <div class="col-md-2 sdbox-col"><label><?= Yii::t('ezform', 'Width')?></label></div>
...
<div class="modal-header" style="margin-bottom: 15px;">
    <h4 class="modal-title"><?= Yii::t('ezmodule', 'Search By')?></h4>
</div>
<div class="form-group">
  <div class="row">
    <div class="col-md-3"><label><?= Yii::t('ezform', 'Varname')?></label></div>
    <div class="col-md-3 sdbox-col"><label><?= Yii::t('ezform', 'Value')?></label></div>
    <div class="col-md-2 sdbox-col"></div>
  </div>
  <div id="search-item-box">
  <?php
  if(isset($options['search_column']) && is_array($options['search_column']) && !empty($options['search_column'])){
      foreach ($options['search_column'] as $key_search => $value_search) {
          ?>
          <div id="<?=$key_search?>" class="row" style="margin-bottom: 10px;">
            <div class="col-md-3"><input type="text" class="form-control varname-input" name="options[search_column][<?=$key_search?>][varname]" value="<?= isset($value_search['varname'])?$value_search['varname']:''?>"></div>
            <div class="col-md-3 sdbox-col"><input type="text" class="form-control value-input" name="options[search_column][<?=$key_search?>][value]" value="<?= isset($value_search['value'])?$value_search['value']:''?>"></div>
  ...
  <div class="row">
      <div class="col-md-4">
        <a href="#" class="search-items-add btn btn-success"><i class="glyphicon glyphicon-plus"></i> <?= Yii::t('ezmodule', 'Search By')?></a>
    </div>
...
$this->registerJS("
    ...
    $('.action-items-add').on('click', function(){
        $.ajax({
            method: 'POST',
            url: '".Url::to(['/ezmodules/ezmodule-tab/get-widget', 'view'=>'/ezmodule-widget/assets/core_grid/_form_action'])."',
            dataType: 'HTML',
    ...
    $('#search-item-box').on('click', '.search-items-del', function(){
        $(this).parent().parent().remove();
    });
    
    $('.search-items-add').on('click', function(){
        $.ajax({
            method: 'POST',
            url: '".Url::to(['/ezmodules/ezmodule-tab/get-widget', 'view'=>'/ezmodule-widget/assets/core_grid/_form_search'])."',
            dataType: 'HTML',
            success: function(result, textStatus) {
                $('#search-item-box').append(result);
            }
        });
    });
    ...
    function getWidget() {
        $.ajax({
            method: 'POST',
            url: '".Url::to(['/ezmodules/ezmodule-tab/get-widget', 'view'=>'/ezmodule-widget/assets/core_grid/_form_header'])."',
            dataType: 'HTML',
            success: function(result, textStatus) {
