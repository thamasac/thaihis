...
<div class="form-group row">
    <div class="col-md-12">
        <?= Html::label(Yii::t('ezform', 'Fields to display'), 'options[image]', ['class' => 'control-label']) ?>
      <?php
...
<script>
    ...
    $('.action-items-add').on('click', function(){
        $.ajax({
            method: 'POST',
            url: '<?=Url::to(['/ezmodules/ezmodule-tab/get-widget', 'view'=>'/ezmodule-widget/assets/sql_grid/_form_action'])?>',
            dataType: 'HTML',
    ...
    function getWidget() {
        $.ajax({
            method: 'POST',
            url: '<?=Url::to(['/ezmodules/ezmodule-tab/get-widget', 'view'=>'/ezmodule-widget/assets/sql_grid/_form_header'])?>',
            dataType: 'HTML',
            success: function(result, textStatus) {
