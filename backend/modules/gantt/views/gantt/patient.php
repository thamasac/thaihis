        ...
        gantt.config.columns = [
            {
                name: "overdue", label: "", width: 38, template: function (obj) {
                    ...
                    if (obj.actual_date) {
                        ...
                        return '<div class="overdue-indicator"><i class="fa fa-check-circle"></i></div>';
                        //}
...
//            }
        ...
        gantt.load('/gantt/gantt/connector-individual?widget_id=<?= $widget_id ?>&schedule_id=<?= $schedule_id ?>&data_id=<?= $data_id ?>&data_ptid=<?= $data_ptid ?>&group_id=<?= $group_id ?>');

        var dp = new gantt.dataProcessor('/gantt/gantt/connector-individual?widget_id=<?= $widget_id ?>&schedule_id=<?= $schedule_id ?>&data_id=<?= $data_id ?>&data_ptid=<?= $data_ptid ?>&group_id=<?= $group_id ?>');
        dp.init(gantt);
    });
...
Url::to(['/ezforms2/ezform-data/ezform',
    ...
    'reloadDiv' => 'gantt-individual',
    'require' => 'new_task',
]);
?>';
        var initdata = {visit_name: visit_id};
        var data = {ezf_id: ezf_id, target: target, initdata: btoa(JSON.stringify(initdata)), modal: 'modal-ezform-gantt'};
