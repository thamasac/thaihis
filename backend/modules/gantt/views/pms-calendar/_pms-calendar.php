    ...
    'clientOptions' => [
        ...
        'select' => new JsExpression("
		      function(start, end) {
                        ...
                        var modal = 'modal-pms-calendar';
                        var project_id = '$project_id';
                        var subtask_ezf_id = '$subtask_ezf_id';
                        
                        $('#'+modal+' .modal-content').html('<div class=\"sdloader \"><i class=\"sdloader-icon\"></i></div>');
                        $('#'+modal).modal('show');
                        $.ajax({
                            url: '" . Url::to(['/gantt/pms-calendar/addbtn', 'forms' => $forms, 'module_id' => $module_id,]) . "',
                            method: 'GET',    
                            dataType: 'HTML',
                            data: {
                              start: start.unix(),
...
//		  "),
        'events' => new JsExpression("
                      ...
                      var project_id = '$project_id';
                        $.ajax({
                            url: '" . Url::to([
                    '/gantt/pms-calendar/feed', 'search_cal' => $search_cal
                    , 'forms' => EzfFunc::arrayEncode2String($forms)
                    , 'cal' => EzfFunc::arrayEncode2String($cal)
                    , 'tab' => $tab
...
//$viewYear = true;
?>
<?php if (isset($viewYear) && $viewYear == '1'): ?>
    <div class="row">
        <?php for ($i=1;$i<=12;$i++):
            $m = $i<10 ?'0'.$i:$i;
            $initDate = date('Y').'-'.$m.'-'.'01';
            $clientOptions['clientOptions']['defaultDate'] = $initDate;
            $clientOptions['clientOptions']['height'] = 450;
            $clientOptions['options']['id'] = $clientOptions['options']['id'].$i;
            ?>
