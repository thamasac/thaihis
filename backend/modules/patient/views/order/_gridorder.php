...
//                return $model['order_name'] . ($model['external_flag'] == 'Y' ? ' (External Lab)' : '');
            },
            'pageSummary' => 'รวม',
        ...
        [
            ...
            'value' => function ($model)use($btnDisabled) {
                if ($model['order_tran_status'] == '1' and $model['order_tran_cashier_status'] == '' && $btnDisabled == 0) {
        ...
        [
            ...
            'value' => function ($data, $key, $index, $widget)use($target, $btnDisabled) {
                if ($data['order_tran_status'] == '1' && empty($data['order_tran_cashier_status']) && $btnDisabled == 0) :
                    $html = Html::a('<span class="glyphicon glyphicon-trash"></span>', Url::to(['/patient/order/order-delete',
                                        'dataid' => $data['id'],
                                    ]), [
//                                'data-action' => 'delete',
...
//                                'data-confirm' => Yii::t('yii', 'Are you sure you want to delete this item?'),
                ...
                elseif (!empty($data['order_tran_cashier_status'])) :
...
$urlSaveQty = Url::to(['/patient/order/grid-order-change']);
$urlreload = Url::to(['/patient/order/grid-order', 'ezf_id' => $ezf_id, 'target' => $target, 'reloadDiv' => $reloadDiv]);
$this->registerJs("
