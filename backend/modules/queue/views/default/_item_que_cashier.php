...
//            }
//        }
        if (isset($model['cashier_status']) && $model['cashier_status'] == '2') {
            if (isset($params_value[$vParam['name']]) && $params_value[$vParam['name']] == $model[$vParam['model_field']]) {
                $active = ' active';
            }
        } else {
            if (isset($params_value['target']) && $params_value['target'] == $model['targetMain']) {
...
if ($model['visit_admit_an']) {
    $template_content .= "<div class=\"text-center text-warning\">Admit : <strong>{$model['visit_admit_an']} </strong></div>";
}
$content = strtr($template_content, $path_items);
?>

<a href="<?= $current_url.'&page='.$page ?>" data-key="<?= isset($check_active) ? $check_active : isset($model['targetMain']) ? $model['targetMain'] : null ?>" class="list-group-item item <?= $active ?>" style="padding: 5px 5px 5px;<?= isset($model['doc']) ? 'background-color: #ffec87;' : '' ?>">
  <?= $content ?>
</a>
