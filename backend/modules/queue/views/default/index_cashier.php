        ...
        <div id="items-side-scroll-<?= $id ?>" class="row">
            <div class="col-lg-12" id="que-list-view">
                <div class="sidebar-nav-title text-center">
                    ...
                    if (is_array($fields_search_multi)) {
                    ...
                    ?>
                    <div style='margin-top: 2%'> <?= isset($icon) && $icon != '' ? ' <i class="fa ' . $icon . '"></i> ' : '' ?><?= isset($title) && $title != '' ? $title : Yii::t('patient', 'Patients Queue') ?><?= " (" . $dataCount . ")" ?></div>
                </div>
                <?php
                echo \yii\widgets\ListView::widget([
                    ...
                    'layout' => '<div class="list-group" style="margin-bottom:0px;">{items}</div><div class="list-pager">{pager}</div>',
                    'itemView' => function ($model) use (
                        $ezf_main_id, $modelFields, $reloadDiv, $status_field
                        , $bdate_field, $pic_field, $template_content, $que_type, $target, $current_url, $action
                        , $data_columns, $param, $custom_label, $vParamSeach, $params_value
                    ) {

                        return $this->render('_item_que_cashier', [
