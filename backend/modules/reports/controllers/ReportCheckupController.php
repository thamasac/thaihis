...
//            $ezf_table = \backend\modules\patient\Module::$formTableName['order_tran'];
            ...
            if (isset($data[0])) {
                $dataProfile = PatientQuery::getProfilePatientByid($data[0]['ptid']);
            }
        ...
        if (Yii::$app->getRequest()->isAjax) {
            ...
            if ($data) {
                \Yii::$app->response->redirect([
                    '/ezmodules/ezmodule/view',
                    'id' => $ezm_id,
                    'target' => $data['ptid'],
                    'visitid' => $data['visit_tran_visit_id'],
                    'pt_hn' => $data['pt_hn'],
                    'action' => 'que',
                    'que_type' => $que_type,
                    'report_status' => '1',
                ...
                \Yii::$app->response->redirect(['/ezmodules/ezmodule/view', 'id' => $ezm_id]);
            }
        } else {
            throw new NotFoundHttpException(Yii::t('ezform', 'Do not allow this way.'));
        }
...
// report 2 doc
    ...
    public function actionReportToDoc() {
        ...
        if ($que_type == '1') {
            $dataProvider = CpoeFunc::getReportCheckup2Doctor($report_status, $page - 1);
        } else {
            $dataProvider = CpoeFunc::getReportPap2Doctor($page - 1);
        }

        return $this->renderAjax('report2doc', [
                    'dataProviderQ' => $dataProvider,
                    'reloadDiv' => $reloadDiv,
                    'target' => $target,
                    'report_status' => $report_status,
                    'que_type' => $que_type,
        ...
        if (Yii::$app->getRequest()->isAjax) {
            ...
            if ($doc_id) {
                ...
                if ($doc_old <> $doc_id) {
                    ...
                    \backend\modules\patient\controllers\PatientController::saveVisitTran($target, $visit_id, $data);
                }
                if ($que_type == '2') {
                    $ezf_id = \backend\modules\patient\Module::$formID['report_checkup'];
                    $ezf_table = \backend\modules\patient\Module::$formTableName['report_checkup'];
