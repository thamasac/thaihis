...
//        $searchModel['create_date'] = date('d/m/Y');
        ...
        return $this->renderAjax('reportcounter', [
                    'ezf_id' => $ezf_id,
...
//        $pdf = new \common\lib\tcpdf\SDPDF('P', PDF_UNIT, 'A4', true, 'UTF-8', false);
    ...
    private function reportContent($pdf, $visit_id, $pt_hn) {
        ...
        if (/* $data['visit_type'] == '4' */empty($data['ckr_pe']) && !empty($data['ckr_epeptp'])) {
            ...
            $pdf->Cell(195, 0, SDdate::mysql2phpThDateSmall($data['visit_date']), 0, 1, 'C', 0, '', 0, false, 'T', 'M');
            $pdf->ln(2);
//            $pdf->SetFont($pdf->fontName, 'B', $pdf->fontSize);
            $pdf->Cell(5, 0, '', 0, 0, 'L', 0, '', 0, false, 'T', 'M');
            $bdate = str_replace("-", "/", \backend\modules\thaihis\classes\ThaiHisFunc::checkBdate($data['pt_bdate'], "-"));
            $age = \backend\modules\thaihis\classes\ThaiHisQuery::calAge($bdate);
            $pdf->Cell(195, 0, $data['fullname'] . ' อายุ ' . $age . ' ปี  HN ' . $data['pt_hn'], 0, 1, 'L', 0, '', 0, false, 'T', 'M');
...
//        $pdf->SetMargins(20, 15, 0, TRUE);
        ...
        $bdate = str_replace("-", "/", \backend\modules\thaihis\classes\ThaiHisFunc::checkBdate($data['pt_bdate'], "-"));
        $age = \backend\modules\thaihis\classes\ThaiHisQuery::calAge($bdate);
        $pdf->Cell($w[2], 0, $data['fullname'] . ' อายุ : ' . $age . ' ปี  HN ' . $data['pt_hn'] . ' ' . SDdate::mysql2phpThDateSmall($data['visit_date']), 0, 1, 'L', 0, '', 0, false, 'T', 'M');
    ...
    private function reportEkg($pdf, $data, $orderekg_id) {
        ...
        $bdate = str_replace("-", "/", \backend\modules\thaihis\classes\ThaiHisFunc::checkBdate($data['pt_bdate'], "-"));
        $age = \backend\modules\thaihis\classes\ThaiHisQuery::calAge($bdate);
        $pdf->Cell($w[1], 0, $data['fullname'] . ' อายุ : ' . $age . ' ปี   HN ' . $data['pt_hn'] . ' ' . SDdate::mysql2phpThDateSmall($data['visit_date']), 0, 1, 'L', 0, '', 0, false, 'T', 'M');
        $pdf->SetFont($pdf->fontName, 'BU', $pdf->fontSize);
...
//        $pdf->writeHTMLCell($w[1], '', '', '', $text, 0, 1);
    ...
    private function getValue($data, $fieldName) {
        ...
        $modelFields = \backend\modules\ezforms2\models\EzformFields::find()
                        ->where('ezf_id = :ezf_id AND ezf_field_name = :ezf_field_name', [
                            ':ezf_field_name' => $fieldName, ':ezf_id' => $ezf_id])->one();
        $dataInput;
        ...
        return str_replace("Other", "", \backend\modules\ezforms2\classes\EzfUiFunc::getValueEzform($dataInput, $modelFields, $data));
//        return \backend\modules\thaihis\classes\ThaiHisFunc::getValueEzformOld($dataInput, $modelFields, $data);
    }
...
class REPORTCHK extends \common\lib\tcpdf\SDPDF {
    ...
    function Footer() {
        ...
        $this->Cell(97.5, 0, '', 0, 0, 'L', 0, '', 0, false, 'T', 'M');
        $this->Cell(97.5, 0, SDdate::mysql2phpThDateSmall($this->data['report_date']), 0, 1, 'C', 0, '', 0, false, 'T', 'M');
    }
