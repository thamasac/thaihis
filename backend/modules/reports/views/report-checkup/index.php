...
<section id="items-side" class="items-sidebar navbar-collapse collapse" role="complementary" style="margin-left: 45px;width: 220px;">
  ...
  <div id="items-side-scroll" class="row">
    <div class="col-md-12" id="que-list-view">            
        <?php echo CpoeHelper::uiQueReportCheckup($target, $report_status, $que_type, 'que-items', 'doctor', $page, $module_id); ?>
    </div>
  </div>
</section>
  ...
  <div class="row">       
        ...
        if ($target) {
            if ($visit_id) {
                $initdata = [//'ckr_status' => '2', 
                    'ckr_doctor_check' => $data['visit_tran_doctor'],
                    'ckr_doctorverify' => $userProfile['user_id'],
                    'ckr_pt_id' => $target,
                    'ckr_sum_bmi' => backend\modules\cpoe\controllers\ReportCheckupSendController::calBmi($data['bmi_bmi'])['value'],
                    'report_date' => date('Y-m-d H:i:s')];
                $initdata = \backend\modules\ezforms2\classes\EzfFunc::arrayEncode2String($initdata);
                echo CpoeHelper::uiReportCheckup($ezf_id, 
                        $data['report_id'], 
                        $initdata, 
                        'report-checkup', 
                        $visit_id, 
    ...
    <div class="col-md-4 sdbox-col">
      ...
      if ($visit_id) {
          ...
          echo PatientHelper::uiPE('', $visit_id, $target, 'report-checkup-pe', '_pe', FALSE);
...
$url = yii\helpers\Url::to(['/reports/report-checkup/queue-view', 'target' => $target, 'report_status' => $report_status, 'reloadDiv' => 'que-list-view']);
