...
class ReportpdfController extends \yii\web\Controller
{
    ...
    public function actionPrintReport() {
        ...
        $user = \common\models\User::findOne(['id' => $user]);
        if ($user) {
...
//        $pdf = new \common\lib\tcpdf\SDPDF('P', PDF_UNIT, 'A4', true, 'UTF-8', false);
    ...
    private function reportContent($pdf, $visit_id, $pt_hn) {
        ...
        $ezf_table = Yii::$app->controller->module->patientFormTableName['order_tran'];
        ...
        if (/* $data['visit_type'] == '4' */empty($data['ckr_pe']) && !empty($data['ckr_epeptp'])) {
           ...
           // \appxq\sdii\utils\VarDumper::dump($resultLab);
            //content        
            ...
            $pdf->Cell(195, 0, SDdate::mysql2phpThDateSm($data['visit_date']), 0, 1, 'C', 0, '', 0, false, 'T', 'M');
            $pdf->ln(2);
//            $pdf->SetFont($pdf->fontName, 'B', $pdf->fontSize);
...
//            $pdf->Cell(170, 0, $this->getValue($data, 'ckr_summary'), 0, 1, 'L', 0, '', 0, false, 'T', 'M');
            ...
            if ($data['ckr_seedoctor'] == '1') {
                $pdf->Cell(195, 0, '        ควรพบแพทย์ที่ รพ.ใกล้บ้าน/รพ.มะเร็งอุดรธานี เพื่อรับยารักษา ' . $data['ckr_order_drug'], 0, 1, 'L', 0, '', 0, false, 'T', 'M');
            }
            if ($data['ckr_resultcheck'] == '1') {
                $pdf->MultiCell(190, $pdf->spaceHigh, '        ให้นำผลตรวจนี้ไปปรึกษาแพทย์ รพ.ใกล้บ้าน/รพ.มะเร็งอุดรธานี เพื่อตรวจ ' . $data['ckr_resultcheck_detail'], 0, 'L');
            }
        }
        ...
        if ($data['ckr_attach_file']) {
            $ezf_table = Yii::$app->controller->module->patientFormTableName['cytoreport'];
...
//        $pdf->SetMargins(20, 15, 0, TRUE);
        ...
        $pdf->Cell($w[2], 0, $data['fullname'] . ' อายุ : ' . SDdate::getAge(SDdate::dateTh2bod($data['pt_bdate'])) . ' ปี  HN ' . $data['pt_hn'] . ' ' . SDdate::mysql2phpThDateSm($data['visit_date']), 0, 1, 'L', 0, '', 0, false, 'T', 'M');
    ...
    private function reportEkg($pdf, $data, $orderekg_id) {
        ...
        $pdf->SetFont($pdf->fontName, '', $pdf->fontSize);
        $pdf->Cell($w[0], 0, '', 0, 0, 'R', 0, '', 0, false, 'T', 'M');
        $pdf->Cell($w[1], 0, $data['fullname'] . ' อายุ : ' . SDdate::getAge(SDdate::dateTh2bod($data['pt_bdate'])) . ' ปี   HN ' . $data['pt_hn'] . ' ' . SDdate::mysql2phpThDateSm($data['visit_date']), 0, 1, 'L', 0, '', 0, false, 'T', 'M');
        $pdf->SetFont($pdf->fontName, 'BU', $pdf->fontSize);
...
//        $pdf->Cell(97.5, 0, 'ผลการอ่านคลื่นไฟฟาหัวใจ', 0, 1, 'L', 0, '', 0, false, 'T', 'M');
        ...
        $ezf_table = Yii::$app->controller->module->patientFormTableName['report_ekg'];
...
class REPORTCHK extends \common\lib\tcpdf\SDPDF {
    ...
    function Footer() {
        ...
        $this->Cell(97.5, 0, '', 0, 0, 'L', 0, '', 0, false, 'T', 'M');
        $this->Cell(97.5, 0, SDdate::mysql2phpThDateSm($this->data['report_date']), 0, 1, 'C', 0, '', 0, false, 'T', 'M');
    }
