...
class UniqueValidator extends Validator
{
    ...
    private function modelExists($targetClass, $conditions, $model)
    {
        ...
        $query = $this->prepareQuery($targetClass, $conditions, $model);
     ...
     */
    private function prepareQuery($targetClass, $conditions, $model)
    {
        $modelEzf = \backend\modules\ezforms2\classes\EzfQuery::getEzformById($this->ezf_id);
        if(!$modelEzf){
            return false;
        }
        $targetClass = new \backend\modules\ezforms2\models\TbdataAll();
        $targetClass->setTableName($modelEzf['ezf_table']);
        
        $query = $targetClass::find()->where('id<>:id AND rstat not in(0,3)', [':id'=>$model->id]);
