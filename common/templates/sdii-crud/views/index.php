...
<?php if ($generator->indexWidgetType === 'grid'): ?>
	...
	'panelBtn' => Html::a(SDHtml::getBtnAdd(), Url::to(['<?= Inflector::camel2id(StringHelper::basename($generator->modelClass)) ?>/create']), ['class' => 'btn btn-success btn-sm']). ' ' .
...
$('#<?= Inflector::camel2id(StringHelper::basename($generator->modelClass)) ?>-grid-pjax').on('dblclick', 'tbody tr', function() {
    ...
    location.href = '".Url::to(['<?= Inflector::camel2id(StringHelper::basename($generator->modelClass)) ?>/update', 'id'=>''])."'+id;
});	
