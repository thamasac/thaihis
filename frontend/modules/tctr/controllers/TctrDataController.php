...
//        ]);
    ...
    public function actionLoadMarker() {
        ini_set('max_execution_time', 0);
        set_time_limit(0);
        ini_set('memory_limit', '1012M');

        $data = TctrFunction::QueryAllMarkmap();
        \Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
        return $data;
    }
    public function actionShowMarker() {
        ini_set('max_execution_time', 0);
        set_time_limit(0);
        ini_set('memory_limit', '1012M');
        
        $options = isset($_POST) ? $_POST :null;
        $ezf_id = isset($options['ezf_id']) ? $options['ezf_id'] :'1520776142078903600';
        $thaisite = isset($options['thaisite']) ? $options['thaisite'] :'1';
        $othersite = isset($options['othersite']) ? $options['othersite'] :'1';
        $Interventional = isset($options['Interventional']) ? $options['Interventional'] :'1';
        $Observational = isset($options['Observational']) ? $options['Observational'] :'1';
        $type = isset($options['type']) ? $options['type'] :'0';
        $where="";
        
        if($Interventional=='1' and $Observational=='0'){
            $where="and study_type='Interventional' ";
        }elseif($Interventional=='0' and $Observational=='1'){
            $where="and study_type='Observational' ";
        }elseif($Interventional=='1' and $Observational=='1'){
            $where="and (study_type='Interventional' or study_type='Observational') ";
        }elseif($Interventional=='0' and $Observational=='0'){
            $where="and 0 ";
        }
        if($thaisite=='1' and $othersite=='1'){
            $where.="";
        }elseif($thaisite=='1' and $othersite=='0'){
            $where.="and trial_id LIKE 'TCTR%' ";
        }elseif($thaisite=='0' and $othersite=='1'){
            $where.="and trial_id LIKE 'NCT%' ";
        }elseif($thaisite=='0' and $othersite=='0'){
           $where.="and 0 "; 
        }
        if($type<>"0"){
            $where.=" and recruitment_status='$type'";
        } 
        $dropdown= TctrFunction::getAllChoice($ezf_id,'recruitment_status');
        $dropdown[] = ['value' => '0','ezf_choicelabel' => 'All'];
        $data = TctrFunction::QueryAllMarkmap($where);
        VarDumper::dump($data);
